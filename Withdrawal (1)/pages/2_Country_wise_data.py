...
def get_country_report(country):
    ...
    trade_data, bot_data, count_user_trade, count_admin_trade, count_user_bot, count_admin_bot = get_subscription(country)

    if trade_data:
        st.markdown("<h4 style='color: lightgreen; font-size: 20px;'>Trade Subscription Details</h4>", unsafe_allow_html=True)
        trade_admin_table = pd.DataFrame([
            {"Package_Type": k, "Total_Count": count_admin_trade.get(k, 0), "Total_Price": v['by_admin']} 
            for k, v in trade_data.items() if v['by_admin'] > 0
        ])
        trade_user_table = pd.DataFrame([
            {"Package_Type": k, "Total_Count": count_user_trade.get(k, 0), "Total_Price": v['by_user']} 
        ...
        if not trade_admin_table.empty:
            st.table(trade_admin_table)
        else:
            st.write("No trade subscriptions by Admin.")

        st.markdown("<h5 style='font-size: 18px;'>Trade Subscriptions by User:</h5>", unsafe_allow_html=True)
        if not trade_user_table.empty:
            st.table(trade_user_table)
        else:
            st.write("No trade subscriptions by User.")
    else:
        st.write("No trade subscriptions found.")

    if bot_data:
        st.markdown("<h4 style='color: lightblue; font-size: 20px;'>Bot Subscription Details</h4>", unsafe_allow_html=True)
        
        bot_admin_table = pd.DataFrame([
            {"Package_Type": k, "Total_Count": count_admin_bot.get(k, 0), "Total_Price": v['by_admin']} 
            for k, v in bot_data.items() if v['by_admin'] > 0
        ])
        bot_user_table = pd.DataFrame([
            {"Package_Type": k, "Total_Count": count_user_bot.get(k, 0), "Total_Price": v['by_user']} 
        ...
        if not bot_admin_table.empty:
            st.table(bot_admin_table)
        else:
            st.write("No bot subscriptions by Admin.")

        st.markdown("<h5 style='font-size: 18px;'>Bot Subscriptions by User:</h5>", unsafe_allow_html=True)
        if not bot_user_table.empty:
            st.table(bot_user_table)
        else:
            st.write("No bot subscriptions by User.")
    else:
    ...
    user_deposits = pd.DataFrame(deposit_data['user_details'].items(), columns=['Username', 'USD_Recieved'])

    admin_deposits = pd.DataFrame(deposit_data['admin_details'])

    admin_deposits.columns = ['Username', 'USD_Recieved']
