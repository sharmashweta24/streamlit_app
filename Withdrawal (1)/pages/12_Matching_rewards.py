...
import pandas as pd
from core.matching_reward import *
...
def process_record():
    st.title("Matching Transactions Processor")
    ...
    if st.button("Process Matching Transactions"):
        if not start_date or not end_date:
            ...
            csv_filename = datetime.now().strftime("%Y-%m-%d %H-%M-%S")+" matching_rewards.csv"
            st.download_button(
                label="Download CSV",
