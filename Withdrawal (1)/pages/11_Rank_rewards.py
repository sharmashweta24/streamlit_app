...
def process_record():
    st.title("Rank Transactions Processor")
    ...
    if st.button("Process Binary Transactions"):
        if not start_date or not end_date:
            ...
            csv_filename = datetime.now().strftime("%Y-%m-%d %H-%M-%S")+" rank_rewards.csv"
            st.download_button(
                label="Download CSV",
