...
import pandas as pd
from core.compounding_reward import *
...
def process_record():
    st.title("Compounding Transactions Processor")
    ...
    if st.button("Process Compounding Transactions"):
        if not start_date or not end_date:
            ...
            csv_filename = datetime.now().strftime("%Y-%m-%d %H-%M-%S")+" compounding_rewards.csv"
            st.download_button(
                label="Download CSV",
