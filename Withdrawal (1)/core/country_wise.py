...
def user_calculate_deposits(country):
    ...
    for i in user_deposites:
        username = i['Username']
        amount = float(i['USD_Recieved'])
        
        if i['By'] == 'User':
            total_user += amount
            user_details[username] = amount 
        else:
            if username not in admin_details:
                admin_details[username] = 0
            admin_details[username] += amount
            total_admin += amount

    admin_details_list = [{'Username': k, 'Amount': v} for k, v in admin_details.items()]

    return {
        'user_details': user_details, 
        'admin_details': admin_details_list,
...
def get_subscription(country):
    ...
    count_user_trade = defaultdict(int)
    count_admin_trade = defaultdict(int)
    count_user_bot = defaultdict(int)
    count_admin_bot = defaultdict(int)
    ...
    for i in package_details_admin:
        ...
        if package_type == 'TRADE':
            if executed_by == 'Admin':
                ...
                count_admin_trade[name] += 1  
            else:
                trade_summary[name]['by_user'] += price
                count_user_trade[name] += 1 
        elif package_type == 'BOT':
            if executed_by == 'Admin':
                bot_summary[name]['by_admin'] += price
                count_admin_bot[name] += 1  
            else:
                bot_summary[name]['by_user'] += price
                count_user_bot[name] += 1 

    trade_data = dict(trade_summary)
    bot_data = dict(bot_summary)
    count_trade_user = dict(count_user_trade)
    count_trade_admin = dict(count_admin_trade)
    count_bot_user = dict(count_user_bot)
    count_bot_admin = dict(count_admin_bot)

    return trade_data, bot_data, count_trade_user,count_trade_admin, count_bot_user, count_bot_admin
