...
# USER TOTAL TEAM AND BUISNESS VOLUM LEFT
# def get_binary_left_side_total(with_spillover, username):
#     total_business_volume_bot_by_user_left = 0
#     total_business_volume_bot_by_admin_left = 0
#     total_business_volume_trade_by_user_left = 0
#     total_business_volume_trade_by_admin_left = 0
    
#     user_network_0 = [] 
#     user_network_1 = []  
    
#     if with_spillover:
#         userwithoutspilloewer_users_list, user_network_0, user_network_1 = fun_spillower.get_users_network_binary(username)
#     else:
...
#     total_user_business_volume_left = list(user_subscription_qualification.find(
#         {'Username': {'$in': user_network_0}}, 
#         {'_id': 0, 'Username': 1, 'Price': 1, 'Package_Type': 1, 'Executed_By': 1}
#     ))
#     # print(total_user_business_volume_left)

#     total_user_business_volume_left_list = len(total_user_business_volume_left)

#     print(total_user_business_volume_left_list)

#     for record in total_user_business_volume_left:
#         price = float(record['Price'])
#         if record['Package_Type'] == 'TRADE':
#             if record['Executed_By'] == 'User':
#                 total_business_volume_trade_by_user_left += price
#             else:
#                 total_business_volume_trade_by_admin_left += price
#         else: 
#             if record['Executed_By'] == 'User':
#                 total_business_volume_bot_by_user_left += price
#             else:
#                 total_business_volume_bot_by_admin_left += price

#     return (
#         total_business_volume_trade_by_user_left, 
#         total_business_volume_trade_by_admin_left,
#         total_business_volume_bot_by_user_left,
#         total_business_volume_bot_by_admin_left,
#         total_user_business_volume_left_list
#     )
# result = get_binary_left_side_total(True, 'chotadon')
# print(result)
...
def get_team_rank_right(with_spillover,username):
    ...
    return right_rank_counts

# USER TEAM RANK TABLE
def get_team_rank_table(with_spillover,username):
    left_ranks = get_team_rank_left(with_spillover,username)
    right_ranks = get_team_rank_right(with_spillover,username)
    ...
    for rank in sorted(all_ranks):
        # Skip rank "inten" or "Intern" in any case
        # if rank == "Inten" or rank == "Intern":
        #     continue 
...
def total_user_team_deposit_right(with_spillover,username):
    ...
    for i in deposits_right_total:
        amount = float(i.get('USD_Recieved', 0))
        if i.get('By') == 'User':
            total_user_team_deposits_right += amount
        else:
            total_admin_team_deposits_right += amount
...
# print(get_total_user_team_withdrawal(True, 'chotadon'))
...
# print(leader_wise_user_withdrawal('chotadon'))

def leader_wise_user_subscription(username):
        total_user = leader_wise_user_network(username)
        package_details_admin = list(user_subscription_qualification.aggregate([
            {'$match': {'Username': {'$in': total_user}, 'Executed_By': 'Admin'}},
            {'$group': {
                '_id': {'Username': '$Username', 'Package': '$Name'},
                'Total': {'$sum': '$Price'},
                'Count': {'$sum': 1}
            }},
            {'$group': {
                '_id': '$_id.Username',
                'Total': {'$sum': '$Total'},
                'Package': {'$push': {'k': '$_id.Package', 'v': '$Count'}}
            }},
            {'$project': {
                '_id': 0,
                'Username': '$_id',
                'Total': 1,
                'Package': {'$arrayToObject': '$Package'}
            }}
        ]))
        package_details_user = list(user_subscription_qualification.aggregate([
            {'$match': {'Username': {'$in': total_user}, 'Executed_By': 'User'}},
            {'$group': {
                '_id': {'Username': '$Username', 'Package': '$Name'},
                'Total': {'$sum': '$Price'},
                'Count': {'$sum': 1}
        ...
        total_user_subscription_user = list(user_subscription_qualification.aggregate([
            {
                '$match': {'Username': {'$in': total_user}, 'Executed_By': 'User'}
            },
            {
                '$group': {
                    '_id': '$Name',  # Grouping by package name directly
                    'Total': {'$sum': '$Price'},
                    'Count': {'$sum': 1}
                }
            },
        ...
        total_user_subscription_admin = list(user_subscription_qualification.aggregate([
            {
                '$match': {'Username': {'$in': total_user}, 'Executed_By': 'Admin'}
            },
            {
        ...
        return package_details_admin, package_details_user,total_user_subscription_admin, total_user_subscription_user
